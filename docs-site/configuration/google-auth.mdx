...
<Warning>
	Logwiz tracks whether each user has a password set. Users who signed in exclusively via Google
	never had one, so **Reset password** and self-serve password change are disabled for them. To
	give a Google-only user password access after disabling Google auth, delete the user from
