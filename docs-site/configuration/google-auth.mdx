...
<Steps>
  ...
  <Step title="Enter credentials in Logwiz">
    In Logwiz, go to **Administration → Authentication** and find the **Google Auth** section. Enter:
...
<Warning>
	Users who signed in exclusively via Google have no password set, and Logwiz blocks password
	reset for any account still linked to a Google identity. To give a Google-only user password
	access after disabling Google auth, delete the user from **Administration → Users** and invite
	them again with a new credential account.
</Warning>
