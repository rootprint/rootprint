...
| `ORIGIN`                       | Yes      | `http://localhost:8282`   | Canonical public URL of the Logwiz instance. Required so invite links and Better Auth callbacks resolve correctly. Set to `http://localhost:5173` for local dev (Vite), keep the default for the Docker image, or use your public URL (e.g. `https://logs.example.com`) behind a reverse proxy. |
...
services:
  logwiz:
    ...
    environment:
      ...
      # Public URL — required (defaults to http://localhost:8282)
