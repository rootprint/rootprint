...
services:
  ...
  logwiz:
    ...
    environment:
      ...
      LOGWIZ_ADMIN_USERNAME: logwiz
      LOGWIZ_ADMIN_PASSWORD: logwiz
    healthcheck:
...
Customize the deployment by setting environment variables under the `environment` key of the `logwiz` service. See the [environment variables reference](/configuration/environment-variables) for the full list, including admin credentials, rate limits, and authentication settings.
