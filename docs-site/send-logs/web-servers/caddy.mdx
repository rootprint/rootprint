...
<Steps>
  ...
  <Step title="Configure Caddy to log to stdout">
    In containerized setups Caddy should write JSON to stdout so Docker captures it as the
    container's log stream. Edit your `Caddyfile` — the `log` block goes on every site whose
    access log you want shipped. Leave the default JSON encoder in place.
    ...
    The Caddy container will be restarted in step 4 below.
  ...
  <Step title="Start Vector and reload Caddy">
    ...
    under `services:`, which can differ from `container_name`.
