...
<Warning>
	`force` triggers an extra commit per request. Using it on small, frequent batches creates many tiny
	splits and significantly degrades indexing performance. Reserve it for one-off scripts or tests.
</Warning>
