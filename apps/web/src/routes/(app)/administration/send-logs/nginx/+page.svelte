	...
	{:else}
		<ol class="flex flex-col">
			<SendLogsStep number={1}>
				<div>
					...
					<p class="mt-1 text-sm text-base-content/60">
						...
						<InlineCode>/var/log/nginx/error.log</InlineCode> — this wizard targets those defaults, no
						<InlineCode>nginx.conf</InlineCode> changes required. If you have replaced the default
			...
			<SendLogsStep number={4}>
				<div>
					...
					<p class="mt-1 text-sm text-base-content/60">
						...
						<InlineCode>www-data:adm</InlineCode> with mode <InlineCode>640</InlineCode>, so add the <InlineCode
							>vector</InlineCode
						> user to the <InlineCode>adm</InlineCode> group.
					</p>
				</div>
			...
			<SendLogsStep number={6}>
				<div>
					...
					<p class="mt-1 text-sm text-base-content/60">
						...
						<InlineCode>http://localhost/</InlineCode> with whichever site URL nginx is configured for.
					</p>
				</div>
			...
			<VerifySearchStep number={7}>
				Search for <InlineCode>service.name:nginx</InlineCode>. Records typically appear within 5–10
