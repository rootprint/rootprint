...
${vectorOtlpSinkSnippet({ ctx, inputs: 'nginx_logs' })}`;
		...
		return [
			...
			},
			{
				...
				body:
					'Save this at /etc/vector/vector.yaml. The endpoint and token are prefilled — ' +
			...
			},
			{
				...
				body:
					'Vector runs as its own user; add it to the adm group so it can read /var/log/nginx/*.',
				snippets: [
