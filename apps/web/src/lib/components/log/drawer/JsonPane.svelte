...
<div class="flex h-full min-h-0 flex-col p-3">
  <div
    class="relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-md border border-base-content/10 bg-base-200/50"
  >
    <div
      class="flex items-center justify-between border-b border-base-content/10 bg-base-200 px-3 py-1.5"
    >
    ...
    <div class="json-pane min-h-0 flex-1 overflow-auto px-3 py-2 text-xs leading-relaxed">
      {#if html}
        {@html html}
      {:else}
        <pre class="font-mono text-base-content/60">{pretty}</pre>
      {/if}
    </div>
  </div>
</div>
