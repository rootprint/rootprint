...
- There is **no span search and no trace list**. The only read endpoint is `GET /api/traces/:traceId`; a trace is reached from a log carrying its id, or by pasting the id into the log search box. Span search was cut deliberately — it duplicated the log explorer's shell without its field panel, saved views, or shares. When it returns it belongs in the explore page as a trace-aware index, not as a second explorer.
