...
- Spans live in one Quickwit index, the span store, named by `config.traceIndexId` (env `TRACE_INDEX_ID`, default `otel-traces-v0_9`). The only trace-related column in `index_settings` is `trace_id_field` — the per-log-index path to a trace id in a log document; there is no trace-side config.
- There is **no span search and no trace list**. The only read endpoint is `GET /api/traces/:traceId`; a trace is reached from a log that carries its id, or by pasting the id into the log search box. Span search was cut deliberately — it duplicated the log explorer's shell without its field panel, saved views, or shares. When it returns it belongs in the explore page as a trace-aware index, not as a second explorer.
- `GET /api/traces/:traceId` (`routes/traces.ts`) is a top-level mount next to `/api/indexes`, not nested under it — no index in the path, no per-request index binding, gated only by `requireUserOrPersonalKey({ logs: ['read'] })`. Trace ids are not authorization.
- It is a raw `trace_id:<hex>` search with **no time range**, not Quickwit's Jaeger API — that API forced a `now - lookback_period_hours` window (72h default) and made older traces findable but unopenable. Capped at `MAX_TRACE_SPANS` (2,000) documents, or a hit missing `span_id`/`span_start_timestamp_nanos`; either sets `truncated: true`. The cap is a payload ceiling, not a paging limit: a whole trace is rendered from one response, and 10k spans measured ~22 MB. Spans are deduplicated by `span_id`, and resource identity is `{service_name, resource_attributes}` because Quickwit stores `service.name` outside `resource_attributes`.
- The read is recorded through `withSearchAudit`, with `spans.length` as its result count; validation, authentication, and rate-limit failures that occur before the handler follow the same unaudited behavior as log search.
- `REDUNDANT_ATTRIBUTES` (`otel.status_code`, `error`) are stripped from every span's attributes: both restate `isError`, which the span already carries, and Envoy/Istio set `error` on ~every failing span.
