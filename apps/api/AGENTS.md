...
- Spans live in one Quickwit index, the span store, resolved by `traceIndexFor()` (`lib/quickwit.ts`) from `config.traceIndexId` (env `TRACE_INDEX_ID`, default `otel-traces-v0_9`). `index_settings` keeps only `trace_id_field` — the per-log-index path to a trace id in a log document; there's no trace-side config left.
- `GET /api/traces/*` (`routes/traces.ts`) is a top-level mount next to `/api/indexes`, not nested under it — no index in the path, no per-request index binding. Covers `:traceId`, `search`, `histogram`, `services`, `operations`, gated only by `requireUserOrPersonalKey({ logs: ['read'] })`. Trace ids are not authorization.
...
- `POST /v1/traces` routes to `traceIndexFor()` for every ingest key, regardless of which log index it was created against — no per-key destination is stored or resolved. `createApiKey` refuses to anchor a key to the span store itself (`400 INDEX_IS_TRACE_INDEX`), since that would let it write log documents into it.
- `isTraceIndex` on `IndexSummary`/`IndexDetail` is derived (`indexId === traceIndexFor()`), not stored — read-only, and exists only to keep the span store out of the log explorer and index pickers.
