...
export function canAccessIndex(
  visibility: IndexVisibility,
  isAdmin: boolean,
): boolean {
...
export async function listIndexes(
  ...
  const rows = ids.length
    ? await db
        .select()
        .from(indexSettings)
        .where(inArray(indexSettings.indexId, ids))
    : [];
...
export async function setSourceEnabled(
  ...
  await withNotFound(
    () => qw.index(indexId).toggleSource(sourceId, enabled),
    "Source not found",
  );
}
...
export async function deleteSource(
  ...
  await withNotFound(
    () => qw.index(indexId).deleteSource(sourceId),
    "Source not found",
  );
}
...
export async function deleteIndex(
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      await tx
        .delete(indexStatsSnapshot)
        .where(eq(indexStatsSnapshot.indexId, indexId));
      await tx
        .delete(userPreference)
        .where(eq(userPreference.indexName, indexId));
      await tx
