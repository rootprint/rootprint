...
transforms:
  parse_nginx:
    ...
    source: |
      ...
      if .nginx_format == "error" {
        ...
        if sev == "debug"  { .severity_number = 5;  .severity_text = "DEBUG" }
        else if sev == "info"   { .severity_number = 9;  .severity_text = "INFO" }
        else if sev == "notice" { .severity_number = 10; .severity_text = "NOTICE" }
        else if sev == "warn"   { .severity_number = 13; .severity_text = "WARN" }
        else if sev == "error"  { .severity_number = 17; .severity_text = "ERROR" }
        else if sev == "crit"   { .severity_number = 19; .severity_text = "CRIT" }
        else if sev == "alert"  { .severity_number = 21; .severity_text = "ALERT" }
        else if sev == "emerg"  { .severity_number = 23; .severity_text = "EMERG" }
        else                    { .severity_number = 9;  .severity_text = "INFO" }
      } else {
        .severity_number = 9
        .severity_text   = "INFO"
      }
  ...
  to_otlp:
    ...
    source: |
      ...
      if exists(.timestamp) {
        ts_val, ts_err = to_unix_timestamp(to_timestamp!(.timestamp), unit: "nanoseconds")
        if ts_err == null {
          ts_nano = ts_val
        }
      }
